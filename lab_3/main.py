...
def film_in_category(category:Union[int,str])-> Union[Iterator[DataFrame], DataFrame, None]:
...
def film_in_category_case_insensitive(category:Union[int,str])-> Union[Iterator[DataFrame], DataFrame, None]:
    if isinstance(category, int):
        df = pd.read_sql("SELECT film.title, language.name languge, category.name category "
                         "FROM film " 
                         "JOIN film_category ON film.film_id = film_category.film_id " 
